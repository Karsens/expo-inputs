...
import { movePhotoFrom } from "../utils";
...
export default class CameraScreen extends React.Component {
  ...
  // [Error: Directory 'file:///Users/progenworks/Library/Developer/CoreSimulator/Devices/E58847C7-0AB0-46EC-8766-67E9091FD14F/data/Containers/Data/Application/476DFE49-34DD-4485-BC57-FBD8861F4A6A/Documents/ExponentExperienceData/%2540wwoessi%252Fcommunify1/photos' could not be created.] Directory exists (probably first check if directory already exists. )
  ...
  takePicture = () => {
    if (this.camera) {
      ...
      //skipProcessing: true <-- possibly fucks up landscape mode pics on android, should test more.
      this.props.navigation.goBack(); // go back before picture is actually saved and moved
    }
  };
  ...
  onPictureSaved = async photo => {
    const to = await movePhotoFrom(photo.uri);

    this.props.navigation.state.params.withUrl({
      uri: to,
      base64: photo.base64
    });
  ...
  collectPictureSizes = async () => {
    if (this.camera) {
      ...
      let pictureSizeId = 0;
      if (Platform.OS === "ios") {
        pictureSizeId = pictureSizes.indexOf("Medium");
      } else {
        // returned array is sorted in ascending order - default size is the largest one
        pictureSizeId = pictureSizes.length - 1;
      }

      // debug on real device in production ftw Alert.alert("",JSON.stringify(pictureSizes));
      this.setState({
        pictureSizes,
        pictureSizeId,
